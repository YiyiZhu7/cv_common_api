...
def non_max_suppression(
    ...
    # Settings
    max_wh = 7680  # (pixels) maximum box width and height 每个类别的像素范围
    ...
    boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class 针对每个类别进行offset坐标偏移), scores
    # 如果这个地方NMS的逻辑，使用python+numpy重写，那么不需要考虑torch框架
    i = torchvision.ops.nms(torch.from_numpy(boxes), torch.from_numpy(scores), iou_thres)  # NMS
    n = i.shape[0]  # number of boxes
    if not n:  # no boxes
        return None
    elif n > max_det:  # limit detections
        i = i[:max_det]
    i = i.numpy()
...
class PredictorWithOpenCV(object):
    def __init__(self, model_path, size=640, conf=0.5, iou=0.5, max_det=4, nc=80):
        ...
        self.iou = iou
        self.nc = nc
        self.max_det = max_det  # 一张图像最多允许max_det个检测边框的输出
        self.max_nms = min(self.max_det * 20, 10000)
        self.class_id_2_names = [f'c_{i}' for i in range(nc)]  # 一般情况下应该是随着模型一起恢复的
